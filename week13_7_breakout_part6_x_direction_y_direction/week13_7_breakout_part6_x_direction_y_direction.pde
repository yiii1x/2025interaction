// week13_7_breakout_part6_x_direction_y_direction
// 修改自 week13_6_breakout_part5_many_bricks
// 先用一個磚塊, 一個板子, 一個球
void setup(){
  size(500, 300);
  restart();
...
void restart() {
  for(int i=0; i<10;i++){
    brickAlive[i] = true;
    brickX[i] = 100;//40+i*42;
    brickY[i] = 40+i*10;//100;
  }
  x = 250;
  y = 250;
  dx = 0;
...
void draw(){
  ...
  for(int i=0; i<10; i++){
    if(brickAlive[i]){
      ...
      if(brickY[i] <= y+dy && y+dy <= brickY[i]+10){
        if( (brickX[i]+40 <= x && brickX[i]+40 > x+dx) || (brickX[i] >= x && brickX[i] < x+dx) ){
          brickAlive[i] = false;
          dx = -dx;
        }        
      }
