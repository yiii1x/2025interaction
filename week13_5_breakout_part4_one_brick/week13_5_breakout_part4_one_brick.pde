// week13_5_breakout_part4_one_brick
// 修改自 week13_4_breakout_part3_restart
// 先用一個磚塊, 一個板子, 一個球
void setup(){
  size(500, 300);
}
...
void draw(){
  ...
  if(brickAlive){
    rect(brickX, brickY, 40, 10); // 1個磚塊
    if(brickX < x+dx && x+dx < brickX+40){
      if( (brickY+10 <= y && brickY+10 > y+dy) || (brickY >= y && brickY < y+dy) ){
        brickAlive = false;
        dy = -dy;
      }
    }
